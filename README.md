...
- 예) `백엔드 서버에게 자료를 요청하고 확신`을 기다리는 경우
...
## 2. 더미(Dummy) / Moukup 사이트 (백엔드 자료를 회신)
...
- 백엔드 측에 `Swagger 구성을 요청`하시면 참 좋습니다. (좋아하지는 않는 사람이 있다 처음에 구성할때 힘들어함.)
...
- `PATCH` : 하나의 자료내용 중 `한 부분만 수정`하세요. (DB 에서 자료 한개 중 일부수정)
      ...
      function getPosts() {
        ...
        xhr.onload = function () {
          ...
          if (xhr.status === 200) {
            ...
            // 90% 이상은 백엔드 측에서 잘못된 진행을 한 경우가 대부분
...
### 5.1. Promise 는 2개의 매개변수(즉 콜백하무) 를 받음

- resolve 콜백함수 : 백엔드 정상 결과 처리 함수
- reject 콜백수 : 백엔드 오류 결과 처리 함수
...
-반드시 function 안쪽에 try ~ catch 를 작성
...
- 타입스크립트로 변환하기(`async 는 반드시 Promise 를 리턴 함`)
...
async function getPosts() {
  try {
    ...
    const res2 = await getData<AlbumType[]>("albuns", "GET");
  } catch (error) {
